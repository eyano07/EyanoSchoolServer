...
public class SliceFeesMapper implements Mapper<SliceFeesDto, SliceFees> {
    ...
    @Override
    public List<SliceFees> dtosFromEntities(List<SliceFeesDto> sliceFeesDtos) {
        List<SliceFees> sliceFeesList = new ArrayList<>();
        for (SliceFeesDto sliceFeesDto : sliceFeesDtos) {
            sliceFeesList.add(dtoFromEntity(sliceFeesDto));
        }
        return sliceFeesList;
    }
    /**
