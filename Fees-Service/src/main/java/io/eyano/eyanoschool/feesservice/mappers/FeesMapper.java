...
public class FeesMapper implements Mapper<FeesDto, Fees> {
    ...
    /**
     * Convert a list of FeesDto to a list of Fees entities
     * @param feesDtos : List of Fees entities
     * @return List of Fees entities
     */
    @Override
    public List<Fees> dtosFromEntities(List<FeesDto> feesDtos) {
        List<Fees> feesList = new ArrayList<>();
        for (FeesDto feesDto : feesDtos) {
            Fees fees =  dtoFromEntity(feesDto);
            feesList.add(fees);
        }
        return feesList;
    }
    ...
    public List<FeesDto> entitiesFromDtos(List<Fees> feesList) {
        List<FeesDto> feesDtos = new ArrayList<>();
